...
class Problem(object):
    ...
    def __repr__(self):
        ...
        if not isinstance(self.goal[0],tuple): #normal goal definition
            ...
            print('GOAL_str', 'oneof'+goal_str)
                
        #if(len(self.goal)>1):
            #goal_str='(oneof {})'.format(' '.join(x for x in self._effects[0]))
                     
                     
                     #  Aggiunta da me: per differenziare quando  un singolo goal relativo ad uno specifico dominio è una concatenazione di predicati in and
                                #oppure è composto da un singolo predicato
                #if(len (g[1])> 1): #più predicati in and
                    #goal_str += '(' + g[0] + ' (and ' + ' '.join(repr(pred[1]) for pred in g[1][0]) + '))\n\t\t' #-> IN ORIGINE IL PARSER AVEVA QUESTA RIGA
                    #goal_str += '(' + g[0] + ' (and ' + ' '.join(repr(pred) for pred in g[1]) + '))\n\t\t'
                #if(len(g[1])==1): #un solo predicato 
                     #goal_str += '(' + g[0] + ' (and ' + ' '.join(repr(pred) for pred in g[1]) + '))\n\t\t'


        #sostituisco :goal(and\n con :goal(oneof\n)
