...
#upgrade version
# Compilation of the domain-> add unfair possibility -> complete upgrade
def multi_tier_compilation_domain1(parsed_domain, hierarchy, goal_statement, mtp_domain_file):
    domain = PDDLParser.parse(parsed_domain)

    list_action=[]
    list_trans_action=[]

    for elem in domain.operators:
        if('trans' not in elem.name):
            list_action.append(elem)

        if ('trans'  in elem.name):
            list_trans_action.append(elem)
            # add-trans actions



    domain.operators=list_action
    ...
    original_domain_operators1=[]
    for elem in original_domain_operators:
        if elem.name!='try_to_repair':
            original_domain_operators1.append(elem)
    ...
    for level in hierarchy:
        ...
        for y in hierarchy:
            for aname in original_domain_operators1:
                new_preconditions += [Literal(Predicate('eff_' + y + '_' + aname.name, []),
                                              False)]

        e_lev=[]# add new effects possibility
        e_lev+=[Literal(Predicate('e_' + level, []), True)]
        
        new_preconditions.append(e_lev)
...
########refinements#####---->rimuovere se non funziona
    for lev2 in hierarchy:
        for lev1 in hierarchy:
            name1 = '_'.join(lev1.split('_')[:-1])
            name2 = '_'.join(lev2.split('_')[:-1])
            ...
            if (index1 < index2) and ((name1 != name2) or (name1 == name2 == '')):  # To check the lattice structure
                ...
                #     to ensure that the alignment actions only occur after the actions replacing CE take place
                new_preconditions = [Literal(Predicate('act', []), False)] + \
                                    [Literal(Predicate('l_' + lev2, []), True)] + \
                                    [Literal(Predicate('e_' + lev1, []), True)]
                for y in hierarchy:
                    for aname in original_domain_operators1:
                        new_preconditions += [Literal(Predicate('eff_' + y + '_' + aname.name, []),
                                                      False)]
                ...
                # 3. set to false l_Dx and set to true l_Dy
                new_effects = [(1.0, Literal(Predicate('act', []), True))] + \
                              [(1.0, Literal(Predicate('l_' + lev1, []), True))] + \
                              [(1.0, Literal(Predicate('l_' + lev2, []), False))]
                for x in hierarchy:
                    new_effects += [(1.0, Literal(Predicate('e_' + x, []), False))]

                # *Add the action to the domain*
                domain.add_action('upgrade_' + lev2 + '_' + lev1, [], new_preconditions, [new_effects])
###########################################
    ...
    for action in original_domain_operators1:
        ...
        for domain_effects in action.effects:
            ...
            if effects not in different_effects:# commentare questa riga se ho azioni uguali per domini diversi->es change tire
        ...
        for lev in hierarchy:
            ...
            for c in original_domain_operators1:
            ...
            for domain_l in action.effects:
                u_eff=[(1.0,Literal(Predicate('u_'+action.name,[]),True))]
            new_effects+=[u_eff]
    ...
    with open(mtp_domain_file, 'w') as f:
        ...
        for elem in list_trans_action:
            domain.add_action(elem.name, elem.params, elem.precond, elem.effects)
...
#basic version
def multi_tier_compilation_domain2(domain, hierarchy, goal_statement, mtp_domain_file):
    parsed_domain = PDDLParser.parse(domain)
    ...
    for elem in parsed_domain.operators:
        if ('trans' not in elem.name):
            list_action.append(elem)
        ...
        if ('trans' in elem.name):
            list_trans_action.append(elem)
            # add-trans actions
    parsed_domain.operators = list_action

    # ..:: PREDICATES ::..
    parsed_domain.add_pred('end', [])
    parsed_domain.add_pred('act', [])
    for level in hierarchy:
        parsed_domain.add_pred('l_' + level, [])
        parsed_domain.add_pred('e_' + level, [])
        for a in parsed_domain.operators:
            parsed_domain.add_pred('eff_' + level + '_' + a.name, [])
    for o in parsed_domain.operators:
        parsed_domain.add_pred('u_' + o.name, [])

    # ..:: ACTIONS ::..
    original_domain_operators = list(parsed_domain.operators)
    original_domain_operators1=[]
    for elem in original_domain_operators:
        if elem.name!='try_to_repair':
            original_domain_operators1.append(elem)
    # We clean the original actions
    parsed_domain.operators = []
    ...
    for level in hierarchy:
        ...
        # *Add the action to the domain*
        parsed_domain.add_action('continue_' + level, [], new_preconditions, [new_effects])
    ...
    for lev1 in hierarchy:
        for lev2 in hierarchy:
            ...
            if (index1 < index2) and ((name1 != name2) or (name1 == name2 == '')):  # To check the lattice structure
                ...
                # *Add the action to the domain*
                parsed_domain.add_action('degrade_' + lev1 + '_' + lev2, [], new_preconditions, [new_effects])
    ...
    for lev2 in hierarchy:
        for lev1 in hierarchy:
            ...
            if (index1 < index2) and ((name1 != name2) or (name1 == name2 == '')):  # To check the lattice structure
                ...
                # *Add the action to the domain*
                parsed_domain.add_action('refine_' + lev2 + '_' + lev1, [], new_preconditions, [new_effects])
###########################################
    ...
    for action in original_domain_operators1:
        ...
        for e, d in zip(different_effects, higher_domain_effects):
            new_effects = [(1.0, Literal(Predicate('eff_' + d + '_' + action.name, []), True))] + \
                          [(1.0, Literal(Predicate('act', []), False))]
            final_effects.append(new_effects)
        parsed_domain.add_action(action.name + '_unfair_', [], new_preconditions,
                          final_effects)
        ...
        for lev in hierarchy:
            ...
            # We add the action without conditional effects
            parsed_domain.add_action(action.name + '_' + lev, action.params,
                              new_preconditions, new_effects)
        ...
        for e, d in zip(different_effects, higher_domain_effects):
            ...
            for d_prime in hierarchy:
                ...
                if (index1 >= index2) and (
                    ...
                    eff += [(1.0, Literal(Predicate('act', []), False))]
                    eff += [(1.0, Literal(Predicate('u_' + action.name, []), False))]
                    parsed_domain.add_action(action.name + '_eff_' + d + '_explained_by_' + d_prime,
                                      action.params, precond, [eff])
    ...
    for goal in goal_statement:
        ...
        for p in goal_predicates:
            ...
            # Otherwise, we have to create as many check_goal actions as parts of the or
                ...
                parsed_domain.add_action('check_goal_' + domain_level, [],
                                  new_preconditions,
                                  [new_effects])
        ...
        if not or_statement:
            ...
            parsed_domain.add_action('check_goal_' + domain_level, [], new_preconditions, [new_effects])

    # Write the domain
    with open(mtp_domain_file, 'w') as f:
        for elem in list_trans_action:
            parsed_domain.add_action(elem.name, elem.params, elem.precond, elem.effects)

        print(repr(parsed_domain),file=f)
...
#basic version-> only downgrade
def multi_tier_compilation_domain5(domain, hierarchy, goal_statement, mtp_domain_file):
    # ..:: PREDICATES ::..
    domain.add_pred('end', [])
    domain.add_pred('act', [])
    for level in hierarchy:
...
#
def multi_tier_compilation_domain3(domain, hierarchy, goal_statement, mtp_domain_file):
    
    # ..:: PREDICATES ::..
    domain.add_pred('end', [])
    domain.add_pred('act', [])
    domain.add_pred('choose', []) #add a new predicate
    for level in hierarchy:
        domain.add_pred('l_' + level, [])
        domain.add_pred('e_' + level, [])
        for a in domain.operators:
            if(a.name != 'try_to_repair'):
                domain.add_pred('eff_' + level + '_' + a.name, [])
            #if(a.name == 'try_to_repair'):
                #domain.add_pred('choose', [])

    for o in domain.operators:
        if(o.name != 'try_to_repair'):
            domain.add_pred('u_' + o.name, [])
    ...
    for level in hierarchy:
        ...
        # *Add the action to the domain*
        if( int(level[-1]) !=1):
            new_eff= [(1.0, Literal(Predicate('choose',[]), False))] 
            new_effects+=new_eff
            new_precond=[Literal(Predicate('choose',[]), True)] 
            new_preconditions+=new_precond
                         
            domain.add_action('continue_' + level, [], new_preconditions, [new_effects])

        
        if( int(level[-1]) ==1):
            new_precond=[Literal(Predicate('choose',[]), False)] 
            new_precond+=[Literal(Predicate('scratch',[]), True)] 
            new_preconditions+=new_precond
            domain.add_action('continue_' + level, [], new_preconditions, [new_effects])
    ...
    for lev2 in hierarchy:
        for lev1 in hierarchy:
            ...
            if (index1 < index2) and ((name1 != name2) or (name1 == name2 == '')):  # To check the lattice structure
                ...
                # *Add the action to the domain*
                domain.add_action('refine_' + lev2 + '_' + lev1, [], new_preconditions, [new_effects])
###########################################
    ...
    for action in original_domain_operators:
        if(action.name=='try_to_repair'):
            ...
            #prendo le precondizioni del labelled domain
            new_preconditions= action.precond
            print('precod:',new_preconditions)
            #ne aggiungo altre relative al dominio compilato
            new_preconditions += [Literal(Predicate('act', []), False)]+\
                                [Literal(Predicate('l_'+lev , []), True)]+\
                                [Literal(Predicate('e_d'+str(int(lev[-1])+1),[]), True)]+\
                                [Literal(Predicate('choose',[]), True)]
            ...
            #action.effects[0]+=(1.0,(Literal(Predicate('choose',[]), True)))
            
            action.effects[0]+=[(1.0,(Literal(Predicate('choose',[]), False)))]
            action.effects[1]+=[(1.0,(Literal(Predicate('choose',[]), False)))]
            new_effects=action.effects

            print('new',new_effects)
            
            final_effects.append(new_effects)
            
            param=action.params
            domain.add_action(action.name , param, new_preconditions,
                          new_effects)
        ...
        # 2. action's unfair predicate
            new_preconditions = [Literal(Predicate('act', []), True)] + \
                            [Literal(Predicate('u_' + action.name, []), True)]+\
                             [Literal(Predicate('choose', []), False)] #add not choose as precondition of unfair actions
        ...
        # It represents the set of fair effects from a given domain level
            ...
            # 4. Negation of the unfair predicates
                new_preconditions = action.precond + \
                                [Literal(Predicate('l_' + current_levels[-1], []), True)] + \
                                [Literal(Predicate('act', []), True)]+\
                                [Literal(Predicate('choose', []), False)] #add not choose come precondition of general actions

                                
            
                for c in original_domain_operators:
                    if( c.name!='try_to_repair'):
                        new_preconditions += [Literal(Predicate('u_' + c.name, []), False)]
            ...
            #   
                        ...
                        if(action.name!='try_to_repair'):
                            ...
                            if (index1 >= index2) and  (
                                ...
                                precond = list(action.precond)
                                precond += [Literal(Predicate('eff_' + d + '_' + action.name, []), True)]
                                precond+=[Literal(Predicate('choose', []), False)]
                                aux = generate_explicability_formula(d_prime, e, different_effects, higher_domain_effects,
                                                         hierarchy)
                                for x in aux:
                                    precond += [x]
                        ...
                        #eff = list(action.effects[hierarchy.index(d)][1])
                                ...
                                if(int(d_prime[-1])==1):
                                    ...
                                    action.params, precond, [eff])
                                if(int(d_prime[-1])!=1):
                                    eff+=[(1.0, Literal(Predicate('choose', []), True))]
                                    domain.add_action(action.name + '_eff_' + d + '_explained_by_' + d_prime,
                                    action.params, precond, [eff])
    ...
    # For each goal statement in the problem, we will have a *CHECK_GOAL* action that checks that a given goal
    # is achieved when being at a certain domain level
    print('goal2',goal_statement[0][1])
    print('goal1',goal_statement[1][1])
    print(len(goal_statement))

    
    for i in range(len(goal_statement)):
        goal=goal_statement[i][1]
        
    goal2=goal_statement[0][1]
    goal1=goal_statement[1][1]
    for goal in goal_statement:
        domain_level = goal[0]
        ...
        for p in goal_predicates:
            ...
            # Otherwise, we have to create as many check_goal actions as parts of the or
                ...
                new_preconditions += [Literal(Predicate('act', []), True)]
                new_preconditions+=[Literal(Predicate('choose', []), False)]
...
def multi_tier_compilation_domain4(domain, hierarchy, goal_statement, mtp_domain_file):
    
    goal_high=list(goal_statement[0][1])
    print('goal_high',goal_high[0])
    
    print(len(goal_high[0]))
    list_fluents=list()
    
    for i in range(1,len(goal_high[0])):
        for pred in goal_high:
            list_fluents.append(pred[i][1])

    print(list_fluents)
    lista_fluents=list()
    for elem in list_fluents:
        lista_fluents.append(str(elem).split(' ')[1])
    print(lista_fluents)
    ...
    for level in hierarchy:
        ...
        domain.add_pred('e_' + level, [])
        for a in domain.operators:
            if("try_to_repair" not in a.name ):
                domain.add_pred('eff_' + level + '_' + a.name, [])
            

        
    for o in domain.operators:
        if("try_to_repair" not in o.name ):
            domain.add_pred('u_' + o.name, [])
    ...
    for elem in original_domain_operators:
        if(elem.name=='climb'):
            param=copy.copy(elem.params) #extract  param for manteinance actions
    ...
    #add manteinance actions
    new_preconditions = [Literal(Predicate('choose', []), True)] +\
                        [Literal(Predicate('l_d1', []), True)]+\
                        [Literal(Predicate('at ?p', []), True)]+\
                         [Literal(Predicate('act', []), False)]
    
    final_eff=[]
    
    
    
    for elem in lista_fluents:
        new=[Literal(Predicate(str(elem), []), True)] #add "damaged predicate"
        new_preconditions+=new

        
        
        eff1=[(1.0, Literal(Predicate(str(elem), []), False))]+\
            [(1.0, Literal(Predicate('choose', []), False))]
        
        eff2=[(1.0, Literal(Predicate(str(elem), []), True))]+\
            [(1.0, Literal(Predicate('choose', []), False))]
        ...
        domain.add_action('try_to_repair' + '_' + str(elem) , param, new_preconditions, final_eff)
        new_preconditions=[Literal(Predicate('choose', []), True)] +\
                          [Literal(Predicate('l_d1', []), True)]+\
                          [Literal(Predicate('act', []), False)]+\
                          [Literal(Predicate('at ?p', []), True)]
    ...
    for level in hierarchy:
        ...
        for y in hierarchy:
            ...
            for aname in original_domain_operators:
                print(aname.name)
                print(aname.params)
                print(type(aname.params))
                #print("try_to_repair" not in aname.name)
                if( "try_to_repair" not   in aname.name  ):
                    new_preconditions += [Literal(Predicate('eff_' + y + '_' + aname.name, []),
                                              False)]
                #if(aname.name != 'try_to_repair'):
    ...
    for lev2 in hierarchy:
        for lev1 in hierarchy:
            ...
            if (index1 < index2) and ((name1 != name2) or (name1 == name2 == '')):  # To check the lattice structure
                ...
                for y in hierarchy:
                    for aname in original_domain_operators:
                        if("try_to_repair "  in aname.name):
                            new_preconditions += [Literal(Predicate('eff_' + y + '_' + aname.name, []),
                                                      False)]
...
###########################################
    ...
    for action in original_domain_operators:
        ...
        if("try_to_repair" not  in action.name):
        ...
        # It represents the set of fair effects from a given domain level
            ...
            # 4. Negation of the unfair predicates
                ...
                for c in original_domain_operators:
                    ...
                    if( 'try_to_repair' not in c.name):
                        new_preconditions += [Literal(Predicate('u_' + c.name, []), False)]
...
def multi_tier_compilation_problem(problem, mtp_problem_file):
    ...
    parsed_problem = PDDLParser.parse(problem)

    domains = []
    for d in parsed_problem.goal:
        domain = d[0]
        domains.append(domain)

    # Update the initial state
    parsed_problem.init.append(Predicate('act'))  # act predicate
    parsed_problem.init.append(Predicate('l_' + (str(domains[0]))))  # initial domain level (top model in the hierarchy)

    # Update the goal
    original_goal = list(parsed_problem.goal)
    parsed_problem.goal = [Predicate('end')]  # new goal conditio

    # Write the problem
    with open(mtp_problem_file, 'w') as f:
        print(repr(parsed_problem), file=f)
