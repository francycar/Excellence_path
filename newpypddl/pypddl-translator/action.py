...
class Action(object):
    ...
    def __det_effect_to_str(effect):
        ...
        elif len(effect) == 1:#-> quella di base
            effect_str = ' '.join(repr(e[1]) for e in effect)
        
        
        #elif len(effect) == 1 and isinstance(effect[0],str):# per far funzionare la versione con le etichette
            #effect_str =  ' '.join('('+ e for e in effect)
        
        #elif len(effect) == 1 and not isinstance(effect[0],str):# per far funzionare la versione con le etichette
            #effect_str = ' '.join(repr(e) for e[1] in effect)
        ...
        #elif len(effect) > 1 and  isinstance(effect[0][0],str): # Labeled effects
             effect_str = '(' + effect[0] + ' (and ' + ' '.join(repr(pred[1]) for pred in effect[1][0]) + '))\n\t\t'
            
        

        #elif len(effect) > 1  and type(effect) is str: # Labeled effects -> per aggiungere le etichette
            #effect_str = '(' + effect 
        else:
        ...
        for p in self._precond:
            if not isinstance(p,list):
                ...
                if not isinstance(p,list):
                    ...
                    for l in p:
                        if(type(l) is list):
                            def_precond += '(and {}) '.format(' '.join(repr(x) for x in l))
                        else:
                            def_precond += repr(l)
